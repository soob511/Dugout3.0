				...
				<div class="dropdown me-3">
					...
					<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						<li><a class="dropdown-item"
							href="${pageContext.request.contextPath}/goods/sortingGoods?kind=new&sort=가격높은순">가격
								높은 순</a></li>
						<li><a class="dropdown-item"
							href="${pageContext.request.contextPath}/goods/sortingGoods?kind=new&sort=가격낮은순">가격
								낮은 순</a></li>
						<li><a class="dropdown-item"
							href="${pageContext.request.contextPath}/goods/sortingGoods?kind=new&sort=많이팔린순">많이
								팔린 순</a></li>
						<li><a class="dropdown-item"
							href="${pageContext.request.contextPath}/goods/sortingGoods?kind=new&sort=인기순">인기
								순</a></li>
						<li><a class="dropdown-item"
							href="${pageContext.request.contextPath}/goods/sortingGoods?kind=new&sort=최신순">최신
								순</a></li>
					</ul>
				</div>
			</div>
	...
	<nav aria-label="Page navigation example"
		...
		<ul class="pagination">
			<li class="page-item"><a class="page-link"
				href="?pageNo=1&kind=${param.kind}&sort=${param.sort}"
			...
			<li class="page-item"><c:if test="${pager.groupNo > 1}">
					<a class="page-link"
						href="?pageNo=${pager.startPageNo - 1}&kind=${param.kind}&sort=${param.sort}"
			...
			<c:forEach begin="${pager.startPageNo}" end="${pager.endPageNo}"
				step="1" var="i">
				<li
					class="page-item <c:if test="${pager.pageNo == i}">active</c:if>">
					<a href="?pageNo=${i}&kind=${param.kind}&sort=${param.sort}"
					class="page-link">${i}</a>
				</li>
			</c:forEach>
			<li class="page-item"><c:if
					...
					<a class="page-link"
						href="?pageNo=${pager.endPageNo + 1}&kind=${param.kind}&sort=${param.sort}"
						aria-label="Next"> <span aria-hidden="true"><i
							class="bi bi-chevron-right"></i></span>
					</a>
				</c:if></li>
			<li class="page-item"><a class="page-link"
				href="?pageNo=${pager.totalPageNo}&kind=${param.kind}&sort=${param.sort}"
