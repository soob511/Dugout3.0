...
<script
	src="${pageContext.request.contextPath}/resources/js/cart/cartForm.js"></script>
<link
	href="${pageContext.request.contextPath}/resources/css/cart/cartForm.css"
	rel="stylesheet" />
<link
	...
	<article>
		<div class="cart-box">
			<input class="form-check-input checkbox" type="checkbox" value=""
				id="flexCheckDefault-all"> <label
				class="form-check-label all" for="flexCheckDefault">전체</label>
			<button type="button" value="삭제" id="btnDelete"
				onclick="deleteItem()">삭제</button>

			<c:forEach items="${cartItems}" var="item">
				<div class="cart-list">
					<input class="form-check-input cart-checkbox" type="checkbox"
						id="flexCheckDefault" data-goods-id="${item.goodsId}"> <label
						class="form-check-label" for="flexCheckDefault"></label> <a
						href="${pageContext.request.contextPath}/goods/goodsDetail?goodsId=${item.goodsId}">
						<img
						src="${pageContext.request.contextPath}/goods/getImg?goodsId=${item.goodsId}">
					</a>
					<div class="box">
						<span id="product-name">[${item.goodsTeam}]
							${item.goodsName}</span> <br> <span id="product-price"
							data-price="${item.goodsPrice}"> <fmt:formatNumber
								value="${item.goodsPrice * item.cartCount}" pattern="#,###" />원
						</span><br>

						<button type="button" class="btn btn-light minus"
							onclick="minus(this); updateCart(${item.goodsId})">-</button>
						<input id="boxbox"
							class="btn btn-light cnt product-quantity ${item.goodsId}"
							value="${item.cartCount}" />
						<button type="button" class="btn btn-light plus"
							onclick="plus(this); updateCart(${item.goodsId})">+</button>
					</div>
				</div>
			</c:forEach>
		...
		<div class="pay-form">
			...
			<div class="money-kind">
				<div class="products-pay">
					<span>총 상품 가격</span> <span id="total-price">0원</span>
				</div>
				<div class="delivery-pay">
					<span>총 배송비</span> <span>무료</span>
				</div>
			</div>
			<hr>
			<div class="total-money">
				<span>결제예정금액</span> <span id="pay-price">0원</span>
			</div>
		</div>
		<div class="pay-all">
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" id="modalDeleteBtn"
